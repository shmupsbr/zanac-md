...
Title OPTIONS (session RAM, no SRAM): **Skill** Easy / Normal / Hard (default Normal), **Autofire** Normal / x2 / x3 / x4 / x5 (default Normal = 20-frame period), **Player Ships** 1–5 (default 3), **Player Extend** extra-life schedule plus a matching score bonus (default EVERY 20000 = stock lives + 0%), **Redefine Keys** assigns A/B/C among both / primary only / secondary only without duplicates. Easy caps effective ALC at `0x50` (50% of the `0xA0` rank clamp), multiplies boss TIME by 1.5, and awards 50% of Normal boss/base-clear points; Hard seeds E12E to `0x50` on ALC reset (rank can then rise or ease), halves boss TIME, and doubles those clear bonuses. Player Extend modes change when extra lives fire (every X / 2X / 3X, once, twice, or none). Score bonuses: EVERY X +0%, EVERY 2X +20%, EVERY 3X +30%, X ONCE +60%, X TWICE +50%, 2X ONCE +80%, 2X TWICE +70%, 3X ONCE +100%, 3X TWICE +90%, NO EXTENDS +100% plus a flat 500000 on the HUD at game start (50000 internal; not doubled by the %). X is the stock first extra (E112=0x20 → 2000 internal, 20000 on the HUD trailing-0 placar).
