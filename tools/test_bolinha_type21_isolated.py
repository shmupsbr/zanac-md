...
def main() -> int:
    ...
    print("  share: FRAME_LEAD never sits in a type 21 bank")

    # 5. FRAME_LEAD painted once; later discs point at the pin.
    buf = fn_span(ent, "static const u8 *lead_frame_tiles(const u8 *src, u16 nbytes, u8 want)") or ""
    if not buf or "orb_paint_body_nibbles" not in buf or "orb_keep_body_nibbles" not in buf:
        return fail("lead_frame_tiles must paint FRAME_LEAD PNG once into RAM")
    if "orb_encode_japan_tiles" in buf or "k_japan_pat7" in buf:
        return fail("do not encode Japan pat 7 16x16 for bolinhas")
    pin = fn_span(ent, "static int lead7_pin_ensure(u8 want, u16 *out)") or ""
    if "FRAME_LEAD" not in pin:
        return fail("lead pin must be FRAME_LEAD")
    ...
    if "ebullet_upload_lead7" not in up:
        return fail("NORMAL lead discs must DMA the FRAME_LEAD pin")
    print("  speed: FRAME_LEAD is one paint + pin share")
