...
emits a 48×8 (6×1 tile) 4bpp pattern packed for VDP_loadTileData, plus a
preview PNG. 6×2 cannot sit above FIRE without hitting ROUND; MSX row 17
is the empty pocket between the ROUND digit (16) and FIRE (18). Zanac
wordmark (32×8) sits beside the MD mark (16×8) so both inks survive 8px.

Colors are remapped onto the in-game PAL3 TMS ramp so the dashboard does
not steal PAL1 (objs) or flash with PAL0[1].

Writes:
    res/hud_zanac_md.png     — 48×8 indexed preview (TMS-ish RGB)
    inc/hud_logo.h
    src/data/hud_logo.c
"""
...
def _bbox(src: list[int], sw: int, sh: int, want: set[int]) -> tuple[int, int, int, int]:
    xs: list[int] = []
    ys: list[int] = []
    for y in range(sh):
        base = y * sw
        for x in range(sw):
            if src[base + x] in want:
                xs.append(x)
                ys.append(y)
    if not xs:
        return 0, 0, sw, sh
...
def layout_6x1(src: list[int], sw: int, sh: int) -> list[int]:
    """Zanac wordmark 32×8 | MD mark 16×8. Full-image 48×8 smears both."""
    zx0, zy0, zx1, zy1 = _bbox(src, sw, sh, {TMS_BLUE})
    mx0, my0, mx1, my1 = _bbox(src, sw, sh, {TMS_RED, TMS_GREEN})
    zanac, zw, zh = _crop(src, sw, zx0, zy0, zx1, zy1)
    md, mw, mh = _crop(src, sw, mx0, my0, mx1, my1)
    left = downsample(zanac, zw, zh, 32, 8)
    right = downsample(md, mw, mh, 16, 8)
    out: list[int] = []
    for y in range(8):
        out.extend(left[y * 32:(y + 1) * 32])
        out.extend(right[y * 16:(y + 1) * 16])
    return out
...
def main() -> int:
    ...
    mini = layout_6x1(classified, sw, sh)
 ...
 * 6x1 tiles, PAL3 TMS nibbles (color 0 transparent).
 *
